...
import com.mqttsnet.thinglinks.oauth.granter.CaptchaTokenGranter;
import com.mqttsnet.thinglinks.oauth.properties.CaptchaProperties;
...
import static com.mqttsnet.basic.exception.code.ExceptionCode.CAPTCHA_ERROR;
...
public class CaptchaServiceImpl implements CaptchaService {
    ...
    public R<Boolean> sendSmsCode(String mobile, String templateCode) {
        String encryptMobile = EncryptDecryptUtils.encrypt(mobile);
        if (MsgTemplateCodeEnum.REGISTER_SMS.eq(templateCode)) {
            // 查user表判断重复
            boolean flag = defUserService.checkMobile(encryptMobile, null);
    ...
    public R<Boolean> sendEmailCode(String email, String templateCode) {
        String encryptEmail = EncryptDecryptUtils.encrypt(email);
        if (MsgTemplateCodeEnum.REGISTER_EMAIL.eq(templateCode)) {
            // 查user表判断重复
            boolean flag = defUserService.checkEmail(encryptEmail, null);
