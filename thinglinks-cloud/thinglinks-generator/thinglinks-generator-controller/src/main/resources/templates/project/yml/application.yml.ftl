${projectPrefix}:
  ...
  nacos:
    ...
    group: DEFAULT_GROUP
    discovery:
      ip: ${r"${"}NACOS_DISCOVERY_IP:@nacos.discovery.ip@${r"}"}
  sentinel:
...
spring:
  ...
  application:
    ...
    # 此参数一定要和 thinglinks-gateway-server.yml 文件中配置的 路由前缀(predicates参数) 一致！ （可以参考 system或base 服务）
    path: /${serviceName}
  profiles:
  ...
  config:
    import:
      ...
      - nacos:database.yml?group=${r"${"}${projectPrefix}.nacos.group${"}"}&refresh=true
  ...
  cloud:
    ...
    nacos:
      ...
      discovery:
        namespace: ${r"${"}${projectPrefix}.nacos.namespace${r"}"}
        ip: ${r"${"}${projectPrefix}.nacos.discovery.ip${r"}"}
