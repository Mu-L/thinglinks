...
@AllArgsConstructor
@NoArgsConstructor
@Schema(title = "TcpEventEnum", description = "Tcp事件枚举")
public enum TcpEventEnum {
    ...
    /**
     * Retrieves the corresponding TcpEventEnum for a given name.
     *
     * @param name the name of the MQTT event.
     * @return an Optional of MqttEventEnum.
     */
    public static Optional<TcpEventEnum> getTcpEventEnum(String name) {
        if (StringUtils.isEmpty(name)) {
            return Optional.empty();
        }
        return Stream.of(TcpEventEnum.values())
