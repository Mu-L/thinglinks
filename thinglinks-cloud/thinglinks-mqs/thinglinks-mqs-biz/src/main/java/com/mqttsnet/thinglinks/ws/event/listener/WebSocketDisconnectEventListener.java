 ...
 * @description: webSocket Disconnect 事件监听器
 * @author: ShiHuan Sun
 * @e-mainl: 13733918655@163.com
 * @date: 2023-04-28 11:42
 **/
@Component
@Slf4j
public class WebSocketDisconnectEventListener {
    ...
    /**
     * 发布WebSocket Disconnect事件
     *
     * @param event 事件消息
     */
    @EventListener
    @Retryable(value = {Exception.class}, maxAttempts = 3, backoff = @Backoff(delay = 1000))
    public void publishWebSocketCloseEvent(WebSocketDisconnectEvent event) {
        log.info("Publishing WebSocket Disconnect event: message={}", event.getMessage());
