 ...
 * APP确认升级事件Action类
 *
 * <p>处理静态和动态升级中的APP确认升级事件，主要功能包括：</p>
 * <ul>
 *   <li>验证升级任务和设备信息</li>
 *   <li>验证当前APP确认状态为已确认</li>
 *   <li>触发后续的升级执行流程</li>
 * </ul>
 *
 * <p>该类统一处理静态和动态升级模式下，设备已确认升级后的后续处理场景</p>
 *
 * @author mqttsnet
 * @version 1.0.0
 * @since 2025/11/10
...
public class AppConfirmationConfirmedAction extends BaseOtaUpgradeAction {
     ...
     * 执行APP确认升级事件
     *
     * <p>处理设备升级过程中APP已确认升级的场景，主要逻辑包括：</p>
     * <ul>
     *   <li>验证升级任务和设备信息</li>
     *   <li>检查任务是否需要APP确认</li>
     *   <li>验证当前确认状态为已确认</li>
    ...
    protected void doExecute(OtaUpgradeTaskStatusEnum from, OtaUpgradeTaskStatusEnum to, OtaUpgradeEventEnum event, OtaUpgradeContext context) {
        log.info("AppConfirmationConfirmedAction execute.params... from:{}, to:{}, event:{}, context:{}", from, to, event, context);
        try {
            // 获取升级记录
            ...
            // 处理APP确认升级逻辑
            handleConfirmUpgrade(from, to, event, context, deviceIdentification);

        } catch (Exception e) {
            throw new RuntimeException("处理APP确认升级事件过程中发生异常: " + e.getMessage(), e);
        }
    }
     ...
     */
    private void handleConfirmUpgrade(OtaUpgradeTaskStatusEnum from, OtaUpgradeTaskStatusEnum to,
                                      OtaUpgradeEventEnum event, OtaUpgradeContext context,
                                      String deviceIdentification) {
        log.info("处理APP确认升级事件 - 任务ID: {}, 设备: {}", context.getTaskId(), deviceIdentification);
        ...
        try {
            ...
            // 检查当前确认状态是否为已确认（事件触发时设备应该已经是已确认状态）
            ...
            if (Objects.isNull(currentStatus) || !currentStatus.equals(OtaTaskRecordAppConfirmStatusEnum.CONFIRMED.getValue())) {
                log.warn("当前APP确认状态不是已确认，无法执行确认后处理 - 任务ID: {}, 设备: {}, 当前状态: {}", context.getTaskId(), deviceIdentification, currentStatus);
                return;
            }

            // 发布OTA任务升级执行事件（确认后的后续处理）
            publishTaskExecutionEventAsync(context.getUpgradeTask(), context.getUpgradeTask().getOtaUpgradesResult(), deviceIdentification);

            log.info("APP确认升级事件处理完成 - 任务ID: {}, 设备: {}", context.getTaskId(), deviceIdentification);
        } catch (Exception e) {
            throw new RuntimeException("处理APP确认升级事件异常: " + e.getMessage(), e);
        }
    }
}