...
public class AclTopicPatternPlaceholderReplacer {
     ...
     */
    public static void replacePlaceholders(List<DeviceAclRuleCacheVO> rules, Optional<DeviceInfoResultVO> deviceInfoOpt) {
        if (CollectionUtil.isEmpty(rules)) {
            return;
        }
        DeviceInfoResultVO deviceInfo = deviceInfoOpt.orElse(null);
        rules.stream()
     ...
     * 替换单个ACL规则中的主题模式占位符
     */
    private static void replacePlaceholder(DeviceAclRuleCacheVO rule, DeviceInfoResultVO deviceInfo) {
     ...
     * 替换字符串中的占位符（内部方法）
     */
    private static String replacePlaceholdersInternal(String pattern, DeviceInfoResultVO deviceInfo) {
     ...
     */
    public static String replacePlaceholders(String pattern, Optional<DeviceInfoResultVO> deviceInfoOpt) {
