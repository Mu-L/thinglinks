...
public class DynamicRetryUpgradeAction extends BaseOtaUpgradeAction {
    ...
    protected void doExecute(OtaUpgradeTaskStatusEnum from, OtaUpgradeTaskStatusEnum to, OtaUpgradeEventEnum event, OtaUpgradeContext context) {
        log.info("DynamicRetryUpgradeAction execute.params... from:{}, to:{}, event:{}, context:{}", from, to, event, context);
        try {
            log.info("动态升级重试 - 任务ID: {}", context.getTaskId());
            // 增加重试次数
            ...
            throw new RuntimeException("处理动态升级重试事件异常: " + e.getMessage(), e);
        }
    }
