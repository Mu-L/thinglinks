  ...
  <PageWrapper contentFullHeight dense>
    <template #headerContent>
      <a-descriptions
        :column="3"
        :title="
          t('devOperation.system.defDict.dict') +
          '【' +
          dict.name +
          '】' +
          t('devOperation.system.defDict.basicInfo')
        "
        size="small"
      >
      ...
      <template #bodyCell="{ column, record }">
        <template v-if="column.dataIndex === 'state'">
          <Tag :color="record.state ? 'success' : 'error'">
            {{ record.state ? t('thinglinks.common.enable') : t('thinglinks.common.disable') }}
          </Tag>
        </template>
  ...
  export default defineComponent({
    ...
    setup() {
      ...
      const [registerTable, { reload, getSelectRowKeys }] = useTable({
        title: () => {
          return t('basic.base.baseDict.dictTitle', { name: dict.name });
        },
        api: page,
        columns: columns(),
        formConfig: {
