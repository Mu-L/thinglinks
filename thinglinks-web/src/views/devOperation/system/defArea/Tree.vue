...
<script lang="ts">
  ...
  export default defineComponent({
    ...
    setup(_, { emit }) {
      ...
      function formatNodes(list: any[] = [], fallbackLevel = ROOT_LEVEL) {
        return list.map((item: any) => {
          const { ...rest } = item;
          const level = String(rest.level ?? fallbackLevel);
          return {
            ...rest,
            key: rest.id,
            title: rest.name,
            level,
            isLeaf: !LEVEL_MAP[level],
            children: undefined,
          } as TreeItem;
        });
      }
      ...
      async function fetch() {
        ...
        treeData.value = formatNodes(result, ROOT_LEVEL);
        // ！！需要首次进入时获取省市两级，则放开下面代码
        // setTimeout(() => {
        //   getTree().filterByLevel(2);
        // }, 0);
      }

      async function handleLoadData(treeNode: TreeItem) {
        if (treeNode.children && treeNode.children.length) {
          return treeNode.children;
        }
        const currentLevel = String(treeNode.level ?? ROOT_LEVEL);
        ...
        const children = formatNodes(result, nextLevel);
        const target = findNodeByKey(treeNode.id, treeData.value);
        if (target) {
      ...
      // 点击树外面的 新增
      function handleAdd() {
        const root = findNodeByKey('0', treeData.value) || { id: '0', level: '10' };
        emit('add', root);
      }
