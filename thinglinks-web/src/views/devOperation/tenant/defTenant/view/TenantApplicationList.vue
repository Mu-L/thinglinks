    ...
    <BasicTable @register="registerTable">
      <template #toolbar>
        <a-button
          ...
          >{{ t('devOperation.tenant.defTenant.authorization') }}
        </a-button>
        <a-button
          v-hasAnyPermission="[RoleEnum.APPLICATION_AUTHORIZE_CANCEL]"
          color="error"
          @click="handleBatchCancelAuthorize"
          >{{ t('devOperation.tenant.defTenant.cancelAuthorization') }}
        </a-button>
      </template>
      <template #bodyCell="{ column, record }">
        <template v-if="column.dataIndex === 'expired'">
          <Tag :color="record.expired ? 'warning' : 'success'">
            {{
              record.expired
                ? t('devOperation.tenant.defTenant.expired')
                : t('devOperation.tenant.defTenant.notExpired')
            }}
          </Tag>
        </template>
        <template v-if="column.dataIndex === 'action'">
          <TableAction
            :actions="[
              {
                label: t('devOperation.tenant.defTenant.authorization'),
                color: 'error',
                auth: RoleEnum.APPLICATION_AUTHORIZE_CANCEL,
                popConfirm: {
                  title: t('devOperation.tenant.defTenant.cancelMsg'),
  ...
  export default defineComponent({
    ...
    setup() {
      ...
      function handleBatchCancelAuthorize() {
        ...
        createConfirm({
          iconType: 'warning',
          content: t('devOperation.tenant.defTenant.cancelMsg'),
          onOk: async () => {
