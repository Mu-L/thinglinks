...
export default defineComponent({
  ...
  setup(_, { emit }) {
    ...
    const [
      registerForm,
      { setFieldsValue, resetFields, updateSchema, validate, clearValidate },
    ] = useForm({
      labelWidth: 120,
    ...
    const [registerDrawer, { setModalProps, closeModal, changeLoading }] =
      ...
      try {
        ...
        if (flag) {
          createMessage.success(
            '初始化数据库、表结构、默认数据成功，请尽快初始化各个服务的链接',
          );
          closeModal();
          emit('success');
        }
