<template>
  ...
  >
    <template #centerFooter>
      <a-button @click="moveToRoot">
        {{ t('devOperation.application.defResource.moveToRoot') }}
      </a-button>
    </template>
    <BasicTree
      ref="treeRef"
  ...
  export default defineComponent({
    ...
    setup(_, { emit }) {
      ...
      function getTree() {
        ...
        if (!tree) {
          throw new Error(t('devOperation.application.defResource.treeLoadFailed'));
        }
        return tree;
      }
        ...
        eachTree(
          ...
          (item, parent) => {
            ...
            if (item.id === unref(current).parentId) {
              item.title = item.name + t('devOperation.application.defResource.cannotMoveToParent');
            } else if (item.id === unref(current).id) {
              item.title = item.name + t('devOperation.application.defResource.cannotMoveToSelf');
            } else if (item.treePath.includes('/' + unref(current).id + '/')) {
              item.title =
                item.name + t('devOperation.application.defResource.cannotMoveToDescendant');
            } else {
      ...
      async function handleSubmit(isMoveRoot = false) {
        try {
          ...
          if (!id) {
            createMessage.error(t('devOperation.application.defResource.pleaseSelectNode'));
            return;
          }
          ...
          if (!isMoveRoot) {
            ...
            const targetName = node
              ? node?.name
              : t('devOperation.application.defResource.rootNode');
            title = t('devOperation.application.defResource.confirmMoveToNode')
              .replace('{name}', unref(current).name)
              .replace('{targetName}', targetName);
          } else {
            parentId = '';
            title = t('devOperation.application.defResource.confirmMoveToRoot').replace(
              '{name}',
              unref(current).name,
            );
          }
          ...
          createConfirm({
            ...
            onOk: async () => {
              try {
                ...
                createMessage.success(t('devOperation.application.defResource.moveSuccess'));
                closeModal();
                emit('success');
              } catch (e) {}
            },
