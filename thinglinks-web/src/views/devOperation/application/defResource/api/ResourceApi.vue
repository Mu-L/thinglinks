    ...
    <BasicTable @register="registerTable">
      <template #toolbar>
        <a-button v-if="type !== ActionEnum.VIEW" type="primary" @click="handleAdd">{{
          t('iot.link.engine.linkage.input')
        }}</a-button>
      ...
      <template #bodyCell="{ column, record }">
        <template v-if="column.dataIndex === 'uri'">
          <Tooltip>
            <template #title>
              {{ t('iot.link.engine.executionLog.action.service') }}：
              {{ record.springApplicationName }} <br />
              {{ t('basic.base.baseDict.name') }}：{{ record.controller }} <br />
              {{ t('basic.msg.extendInterface.name') }}：{{ record.name }} <br />
              {{ t('common.address') }}： {{ record.uri }} <br />
              {{ t('common.requestMethod') }}： {{ record.requestMethod }}
            </template>
