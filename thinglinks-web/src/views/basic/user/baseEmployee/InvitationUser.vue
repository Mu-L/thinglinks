<template>
  <BasicModal
    ...
    :keyboard="true"
    :maskClosable="true"
    :title="t('basic.user.baseUserCenter.inviteUser')"
    @ok="handleSubmit"
  >
    ...
    <Form v-bind="{ labelCol: { span: 8 }, wrapperCol: { span: 16 } }">
      <FormItem name="query" :wrapper-col="{ span: 24, offset: 11 }">
        <a-button type="primary" @click="handleQuery"> {{ t('common.title.query') }} </a-button>
      </FormItem>
      <FormItem
        name="userIdList"
        :label="t('basic.user.baseUserCenter.waitingInviteUser')"
        v-if="userListRef && userListRef.length > 0"
      >
  ...
  export default defineComponent({
    ...
    setup(_, { emit }) {
      ...
      async function handleSubmit() {
        try {
          if (!userIdRef.value) {
            createMessage.warn(t('basic.user.baseUserCenter.pleaseSelectWaitingInviteUser'));
            return;
          }
          ...
          createMessage.success(t('basic.user.baseUserCenter.inviteSuccess'));
