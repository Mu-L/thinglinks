...
<script lang="ts" setup>
  ...
  const labelClassNameItem = reactive([
    { value: 'bg-primary', label: t('basic.user.baseUserCenter.themeBackground') },
    { value: 'bg-white', label: t('basic.user.baseUserCenter.white') },
    { value: 'bg-orange', label: t('basic.user.baseUserCenter.orange') },
    { value: 'bg-gold', label: t('basic.user.baseUserCenter.gold') },
    { value: 'bg-gray', label: t('basic.user.baseUserCenter.gray') },
    { value: 'bg-lightpink', label: t('basic.user.baseUserCenter.lightpink') },
    { value: 'bg-chocolate', label: t('basic.user.baseUserCenter.chocolate') },
    { value: 'bg-tomato', label: t('basic.user.baseUserCenter.tomato') },
  ]);
  const treeProps = { label: 'name', children: 'children', key: 'id' }; // 组件配置
  // const treeData = reactive<TreeResult>();
  const state = reactive({
    treeData: {
      name: t('basic.user.baseUserCenter.orgTree'),
      expand: false,
      key: 0,
      children: [],
    } as TreeResult,
  ...
  function nodeClick(e: Event, data) {
    ...
    if (data && data.id) {
      ...
      state.current = { id: '', name: t('basic.user.baseUserCenter.rootNode') };
      emit('select', { id: '', name: t('basic.user.baseUserCenter.rootNode') });
    }
  }
  ...
  function handleEdit() {
    ...
    if (state.current.id) {
      ...
      createMessage.warn(t('basic.user.baseUserCenter.pleaseSelectNode'));
    }
  }
  ...
  async function batchDelete(ids: string[]) {
    createConfirm({
      iconType: 'warning',
      content: t('basic.user.baseUserCenter.pleaseSelectNodeAndChildren', {
