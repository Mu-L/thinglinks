export interface ProductCommandResponsePageQuery {
...
export interface ProductCommandResponseSaveVO {
  commandId?: string; // 命令ID
  serviceId?: string; // 服务ID
  datatype?: string; // 指示数据类型。取值范围：string、int、decimal
  enumlist?: string; // 指示枚举值。如开关状态status可有如下取值"enumList" : ["OPEN","CLOSE"]目前本字段是非功能性字段，仅起到描述作用。建议准确定义。
...
export interface ProductCommandResponseUpdateVO {
  id: string;
  commandId?: string; // 命令ID
  serviceId?: string; // 服务ID
  datatype?: string; // 指示数据类型。取值范围：string、int、decimal
...
export interface ProductCommandResponseResultVO {
  echoMap?: any;
  id?: string; // id
