      ...
      <div class="card-content">
        <a-row v-if="dataList.length > 0" :gutter="[24, 12]">
          ...
          >
            <div
              class="card_wrap"
              :class="selectNode.serialNumber === record.serialNumber ? 'active' : ''"
              @click="handleSelectNode(record)"
            >
              <div class="left_info">
                <div class="item">
                  <div class="name">{{ record.certName || '未命名' }}</div>
                </div>
                <div class="item">
                  <div class="label">{{
                    t('iot.link.operationMaintenance.cacert.caCertLicense.serialNumber')
                  }}</div>
                  <div class="content" :title="record.serialNumber">{{ record.serialNumber }}</div>
                </div>
                <div class="item">
                  <div class="label">{{
                    t('iot.link.operationMaintenance.cacert.caCertLicense.expirationDate')
                  }}</div>
                  <div class="content" :title="record.notBefore + '-' + record.notAfter">{{
              ...
              />
              <div class="type" :class="record.enabled ? 'error' : 'normal'">
...
<script lang="ts" setup>
  ...
  const handleCancel = () => {
    emits('success', {});
    emits('updateSelectNode', { certName: '' });
    handleSelectNode({ serialNumber: '' });
    closeModal();
  };
  ...
  watch(
    ...
    async (newValue) => {
      ...
      if (dataList.value.length === 0) {
        await getList(current.value, pageSize.value, { serialNumber: newValue });
      }
      const selected = dataList.value.find((item) => item.serialNumber === newValue);
      if (selected) {
