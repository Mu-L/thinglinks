...
export function useColumns(
  propsRef: ComputedRef<BasicTableProps>,
  getPaginationRef: ComputedRef<boolean | PaginationProps>,
  openRemarkModal?: (value: string, title: string) => void,
) {
  ...
  const getViewColumns = computed(() => {
    ...
    const mapFn = (column) => {
      ...
      // 如果启用了复制、查看或 Tooltip，且没有自定义 customRender，则自动生成
      if ((enableCopy || enableView || enableTooltip) && !customRender && !isDefaultAction) {
        const dataIndex = column.dataIndex;
        const columnTitle = column.customTitle || column.title || '';
        const tooltipPlacement = column.tooltipPlacement || 'topLeft';
        // 优先使用传入的 openRemarkModal，如果没有则从 props 获取（向后兼容）
        const openRemarkModalFn = openRemarkModal || unref(propsRef).openRemarkModal;
        ...
        column.customRender = ({ record }) => {
          ...
          if (enableCopy && value) {
            children.push(
              h(CopyOutlined, {
                onClick: () => handleCopyTextV2(displayValue),
                style: { marginRight: '10px', cursor: 'pointer' },
              }),
            );
          }

          if (enableView && value && openRemarkModalFn) {
            children.push(
              h(EyeOutlined, {
                onClick: () => openRemarkModalFn(displayValue, String(columnTitle)),
                style: { marginRight: '10px', cursor: 'pointer' },
              }),
            );
          }
          ...
          const content =
            enableTooltip && value
              ? h(Tooltip, { placement: tooltipPlacement, title: displayValue }, () =>
                  h('span', displayValue),
                )
              : h('span', displayValue);
      ...
      } else if (!customRender && format && !edit && !isDefaultAction) {
      ...
      // edit table
      if ((edit || editRow) && !isDefaultAction && !customRender) {
