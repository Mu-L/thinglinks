<template>
  <n-modal class="go-chart-create-color" v-model:show="modelShowRef" :mask-closable="false" :closeOnEsc="false">
    <n-card :bordered="false" role="dialog" size="small" aria-modal="true" style="width: 900px; height: 720px">
      ...
      <div class="create-content">
        <div class="create-color-setting-box">
          ...
          <div v-else class="no-data go-flex-center">
            ...
            <n-text :depth="3">{{ t('project.create_color_not_selected') }}</n-text>
          </div>
        </div>
          ...
          <div class="color-list">
            <n-space>
              ...
              <n-button
                ...
                @click="createColor"
              >
                <span> {{ t('project.create_color_create') }} </span>
                <template #icon>
                  <n-icon>
              ...
              <n-badge v-if="selectColorId" :show="updateColor !== undefined" dot>
                <n-button class="create-btn" type="info" secondary @click="saveHandle">
                  <span> {{ t('project.create_color_apply_data') }} </span>
                  <template #icon>
                    <n-icon>
            ...
            <n-text v-if="!selectColorId" class="not-data-text" :depth="3">
              {{ t('project.create_color_no_custom') }}
              <n-a @click="createColor">{{ t('project.create_color_create_now') }}</n-a>
            </n-text>
            <!-- 列表 -->
      ...
      <template #action>
        <n-space justify="end">
          <n-button @click="closeHandle">{{ t('project.create_color_operation_complete') }}</n-button>
        </n-space>
      </template>
    </n-card>
  </n-modal>
...
const defaultColor: ColorType = {
  id: getUUID(),
  name: t('project.create_color_unnamed'),
...
const selectHandle = (item: ColorType) => {
  ...
  if (updateColor.value !== undefined) {
    goDialog({
      message: t('project.create_color_unsaved_changes'),
      onPositiveCallback: () => {
...
const deleteHandle = (index: number) => {
  ...
  if (updateColor.value !== undefined) {
    ...
    goDialog({
      message: t('project.create_color_delete_confirm'),
      onPositiveCallback: () => {
        positiveHandle()
      }
    })
...
const saveHandle = (onMessage = true) => {
  ...
  if (index !== -1) {
    onMessage && window.$message.success(t('project.create_color_apply_success'))
    const updateColorPrefix = cloneDeep({ ...updateColor.value, name: updateColor.value.name || t('project.create_color_undefined') })
    ...
    window.$message.error(t('project.create_color_apply_failed'))
  }
}
