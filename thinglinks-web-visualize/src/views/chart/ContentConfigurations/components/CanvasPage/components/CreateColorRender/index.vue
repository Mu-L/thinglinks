  ...
  <div class="create-color-setting" v-if="editColor">
    <n-card :bordered="false" role="dialog" size="small" aria-modal="true">
      <n-space justify="space-between">
        <!-- 名称 -->
        <n-input-group>
          <n-input-group-label>{{ t('project.create_color_render_name') }}</n-input-group-label>
          <n-input
        ...
        <n-tag type="warning">{{ t('project.create_color_render_chart_limit') }}</n-tag>
      </n-space>
      <!-- 颜色 -->
            ...
            <div class="go-flex-items-center" :class="{ select: index === targetColor.index }">
              ...
              <div v-show="index > 5">
                <n-tooltip trigger="hover">
                  ...
                  {{ t('project.create_color_render_delete_color') }}
                </n-tooltip>
              </div>
            </div>
          </div>
          <div>
            <n-button type="primary" secondary @click="addColor">
              <div class="go-flex-items-center">
                <span class="go-mr-4">{{ t('project.create_color_render_add') }}</span>
                <n-icon size="16">
                  <add-icon></add-icon>
                </n-icon>
              </div>
    ...
    <div class="expend-color-box">
      <n-card class="go-mt-3 expend-color" :bordered="false" role="dialog" size="small" aria-modal="true">
        <n-text>{{ t('project.create_color_render_default_expand') }}</n-text>
      ...
      <n-card class="go-mt-3 expend-color" :bordered="false" role="dialog" size="small" aria-modal="true">
        <n-text>{{ t('project.create_color_render_transparent_expand') }}</n-text>
        <n-divider style="margin: 10px 0"></n-divider>
        <n-space :size="[4, 0]" justify="center">
          <div
            class="color-computed-item"
