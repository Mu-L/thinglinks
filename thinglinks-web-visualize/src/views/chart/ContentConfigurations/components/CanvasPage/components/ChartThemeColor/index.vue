  ...
  <div class="go-chart-theme-color">
    <n-card class="card-box" size="small" hoverable embedded @click="createColorHandle">
      <n-text class="go-flex-items-center">
        <span>{{ t('project.theme_custom_color') }}</span>
        <n-icon size="16">
          <add-icon></add-icon>
        </n-icon>
      </n-text>
    ...
    >
      <div class="go-flex-items-center">
        <n-ellipsis style="text-align: left; width: 60px">{{ getThemeName(key, value.name) }} </n-ellipsis>
        <span
...
// 获取主题名称（支持多语言）
const getThemeName = (key: string, defaultName: string): string => {
  // 如果是自定义颜色（UUID），直接返回名称
  if (key.length > 20 || !key.match(/^[a-zA-Z]+$/)) {
    return defaultName
  }
  
  // 将驼峰命名转换为下划线命名（如 purplePassion -> purple_passion）
  const themeKey = key.replace(/([A-Z])/g, '_$1').toLowerCase().replace(/^_/, '')
  const i18nKey = `project.theme_${themeKey}`
  
  // 尝试获取翻译，如果不存在则返回默认名称
  try {
    const translated = t(i18nKey)
    // 如果返回的翻译和 key 相同，说明翻译不存在，返回默认名称
    return translated !== i18nKey ? translated : defaultName
  } catch {
    return defaultName
  }
}
