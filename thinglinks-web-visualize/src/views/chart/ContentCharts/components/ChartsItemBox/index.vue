      ...
      >
        <div class="list-header">
          ...
          <n-text class="list-header-text" depth="3">
            <n-ellipsis>{{ getComponentTitle(item.title) }}</n-ellipsis>
          </n-text>
        </div>
        ...
        <div class="list-bottom">
          <n-text class="list-bottom-text" depth="3">
            <n-ellipsis style="max-width: 90%">{{ getComponentTitle(item.title) }}</n-ellipsis>
          </n-text>
        </div>
        <!-- 遮罩 -->
...
import { componentInstall, loadingStart, loadingFinish, loadingError, JSONStringify, goDialog, getComponentTitle } from '@/utils'
...
const deleteHandle = (item: ConfigType, index: number) => {
  goDialog({
    message: t('project.delete_image_confirm'),
