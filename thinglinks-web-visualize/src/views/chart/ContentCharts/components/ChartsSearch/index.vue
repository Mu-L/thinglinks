      ...
      >
        <template #trigger>
          <n-input-group>
            <n-input
              size="small"
              :placeholder="$t('project.search_components')"
        ...
        <div class="search-list-box">
          <n-scrollbar style="max-height: 500px">
            <n-empty v-show="!searchRes.length" size="small" :description="$t('project.no_components_found')"></n-empty>
            <div
              class="list-item go-flex-items-center go-ellipsis-1"
              v-for="item in searchRes"
              :key="item.key"
              :title="getComponentTitle(item.title)"
            ...
            >
              ...
              <n-text class="list-item-fs" depth="2">{{ getComponentTitle(item.title) }}</n-text>
            </div>
          </n-scrollbar>
...
import { isString, addEventListener, removeEventListener, getComponentTitle } from '@/utils'
...
const chartModeList = computed(() => [
  { label: t('project.single_column'), icon: AlbumsIcon, value: ChartModeEnum.SINGLE },
  { label: t('project.double_column'), icon: GridIcon, value: ChartModeEnum.DOUBLE }
])
...
const searchHandle = (key: string | null) => {
  ...
  searchRes.value = List.filter(
    (e: ConfigType) => !e.disabled && (!key || getComponentTitle(e.title).toLowerCase().includes(key.toLowerCase()))
  )
  setTimeout(() => {
    loading.value = undefined
  }, 500)
...
const selectChartHandle = async (item: ConfigType) => {
  ...
  try {
    ...
    if (item.redirectComponent) {
      ...
      newComponent.chartConfig.title = getComponentTitle(item.title)
    ...
    loadingError()
    window['$message'].warning(t('project.chart_developing'))
  }
}
