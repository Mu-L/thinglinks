<template>
  ...
  >
    ...
    <n-space v-if="reverseList.length === 0" justify="center">
      <n-text class="not-layer-text">{{ $t('project.no_layers') }}</n-text>
    </n-space>
...
const layerModeList = computed(() => [
  { label: t('project.thumbnail'), icon: LaptopIcon, value: LayerModeEnum.THUMBNAIL },
  { label: t('project.text_list'), icon: ListIcon, value: LayerModeEnum.TEXT }
])
