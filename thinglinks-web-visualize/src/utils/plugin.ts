 ...
 export const goDialog = (
  ...
  const typeObj = {
    ...
    [DialogEnum.DELETE]: {
      ...
      message: message || t('global.dialog_delete_confirm')
    },
    // 原有
    [DialogEnum.WARNING]: {
      fn: window['$dialog'].warning,
      message: message || t('global.dialog_operation_confirm')
    },
    [DialogEnum.ERROR]: {
  ...
  const dialog: DialogReactive = typeObj[type || DialogEnum.WARNING]['fn']({
    // 导入其余 NaiveUI 支持参数
    ...params,
    title: title || t('global.dialog_tip'),
    icon: renderIcon(InformationCircleIcon, { size: dialogIconSize }),
    content: typeObj[type || DialogEnum.WARNING]['message'],
    positiveText: positiveText || t('global.confirm'),
    negativeText: closeNegativeText ? undefined : (negativeText || t('global.cancel')),
    // 是否通过遮罩关闭
